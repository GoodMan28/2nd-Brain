...
export function Home() {
    ...
    useEffect(() => {
        const loadSharedContent = async () => {
            ...
            try {
                ...
                const res = await axios.get(`${BACKEND_URL}/api/v1/share/${shareToken}`);
    ...
    const fetchNotes = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${BACKEND_URL}/api/v1/content/getContent`,
                {
                    type: debouncedTypes,
                    tags: debouncedTags
                },
                                ...
                                <NoteCard
                                    ...
                                    onDelete={async () => {
                                        try {
                                            const token = await getToken();
                                            if (!token) return;
                                            await axios.post(`${BACKEND_URL}/api/v1/content/deleteContent`,
                                                { contentId: note.id },
                                                { headers: { "Authorization": `Bearer ${token}` } }
                                            );
